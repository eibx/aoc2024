...
u64 part_1(u8* content, u32 file_size) {
    u64* stones = (u64*)malloc(50000000*sizeof(u64));
    u64 stone_count = 0;

    u64* stones_new = (u64*)malloc(50000000*sizeof(u64));
    u64 stone_new_count = 0;
    ...
    for (u64 i = 0; i < 75; i++) {
        for (u64 j = 0; j < stone_count; j++) {
            u64 number = stones[j];
            if (number == 0) {
                stones_new[stone_new_count++] = 1;
                continue;
            }
